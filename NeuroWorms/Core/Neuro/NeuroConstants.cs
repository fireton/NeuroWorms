using System;

namespace NeuroWorms.Core.Neuro
{
    public static class NeuroConstants
    {
        public static readonly Guid EyeAngleSensorId = new("de059de7-1e33-49e1-b349-87978ab29f06");
        public static readonly Guid EyeDistanceSensorId = new("cb734dcb-32fc-4177-8eb8-659dca46881f");
        public static readonly Guid EyeObjectSensorId = new("0116743f-2803-4d25-a5f0-22c6e22ca729");
        public static readonly Guid LengthSensorId = new("02C916E0-2F10-4EF9-9C98-FFDCA43F6F02");
    }
}
